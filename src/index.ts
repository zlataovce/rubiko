...
const router = Router({ catch: error })
  ...
  .post("/", async (request: IRequest, env: Env, ctx: ExecutionContext): Promise<Response> => {
    ...
    switch (interaction.type) {
      ...
      case InteractionType.ApplicationCommand: {
        const cmd = find(interaction.data.name);
        if (!cmd) {
          return error(400, "Unknown command");
        }

        return json(await cmd.handle(interaction, env, ctx));
      }
    }
  ...
  .put("/", async (request: IRequest, env: Env, _ctx: ExecutionContext): Promise<Response> => {
    ...
    let url = `https://discord.com/api/v10/applications/${env.DISCORD_APPLICATION_ID}`;
    if (request.query.guild) {
      url += `/guilds/${request.query.guild}`;
    }
    url += "/commands";
